...
public class ValidPalindrome {
  ...
  public boolean isPalindrome(String s) {
    ...
    while (start <= end) {
      if (((temp.charAt(start) >= '0' && temp.charAt(start) <= '9') || (temp.charAt(start) >= 'a' && temp
          .charAt(start) <= 'z'))
          && ((temp.charAt(end) >= '0' && temp.charAt(end) <= '9') || (temp.charAt(end) >= 'a' && temp
              .charAt(end) <= 'z'))) {
        ...
        if (!((temp.charAt(end) >= '0' && temp.charAt(end) <= '9') || (temp.charAt(end) >= 'a' && temp
            .charAt(end) <= 'z'))) {
          end--;
        }
      }
